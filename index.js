...
function manipulateUrl(url) {
  ...
  obj["href"] = urlObj.href;
  obj["protocol"] = urlObj.protocol;
  obj["host"] = urlObj.host;
  obj["pathname"] = urlObj.pathname;
  obj["search"] = urlObj.search;
  obj["params"] = Array.from(urlObj.searchParams);
...
function searchParamsURL(url) {
  ...
  URLSearchParams.forEach((value, key) => newMap.set(key, value));
...
function manipulateSearchParams(paramsObj, newUrl) {
  ...
  for (const value of keysObj) {
    urlObj.searchParams.append(value, paramsObj[value]);
  }
...
function getUrlData(url) {
  ...
  obj["origin"] = urlObj.origin;
  obj["hostname"] = urlObj.hostname;
  obj["port"] = urlObj.port;
  obj["username"] = urlObj.username;
  obj["password"] = urlObj.password;
...
function sortUrlParams(url) {
  ...
  let arrEntries = urlObj.searchParams.entries();

  let arr = Array.from(arrEntries);

  arr.sort((elem1, elem2) => elem1[0].localeCompare(elem2[0]));

  urlObj.search = "";

  for (const index in arr) {
    urlObj.searchParams.append(arr[index][0], arr[index][1]);
  }
...
function getURLValues(url) {
  ...
  let urlKey = urlSearchParams.keys();

  let arrKey = Array.from(urlKey);

  for (const value of arrKey) {
    arrValue.push(urlSearchParams.getAll(value));
  }

  return arrValue.flat();
}
...
function getUrlKeys(url) {
  ...
  let urlKey = urlObj.searchParams.keys();
